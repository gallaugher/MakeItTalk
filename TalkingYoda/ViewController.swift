...
class ViewController: UIViewController {
    ...
    let mqttClient = CocoaMQTT(clientID: "yoda", host: "yoda.local", port: 1883)
    ...
    // instead of <piname>.local.

    
    var messages: [String] = [ "clear_your_mind_must_be_if_you_are_to_find_the_villains_behind_this_plot.mp3",
...
extension ViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        print("🥶🥶 Pressed cell # \(indexPath.row) which is the message \(messages[indexPath.row])")
        print("Sending message: \(messages[indexPath.row])")
        mqttClient.publish("yoda/talk", withString: messages[indexPath.row])
    }
